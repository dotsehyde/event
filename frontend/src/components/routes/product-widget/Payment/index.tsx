...
const Payment = () => {
    ...
    const [activePaymentMethod, setActivePaymentMethod] = useState<'STRIPE' | 'OFFLINE' | 'PAYSTACK' | null>(null);
    ...
    }, [isStripeEnabled, isPaystackEnabled, isOfflineEnabled]);
    ...
    const handleSubmit = async () => {
        if (activePaymentMethod === 'STRIPE') {
            handleParentSubmit();
        } else if (activePaymentMethod === 'PAYSTACK') {
            handleParentSubmit();
    ...
    if (!isStripeEnabled && !isOfflineEnabled && !isPaystackEnabled && isOrderFetched && isEventFetched) {
        return (
            <CheckoutContent>
                <Card>
    ...
    return (
        <>
            <CheckoutContent>
                ...
                {((isStripeEnabled && isOfflineEnabled) || (isStripeEnabled && isPaystackEnabled) || (isPaystackEnabled && isOfflineEnabled)) && (
                    <div style={{ marginTop: '20px' }}>
                        <a
                            onClick={() => {
                                if (activePaymentMethod === 'STRIPE') {
                                    setActivePaymentMethod(isPaystackEnabled ? 'PAYSTACK' : 'OFFLINE');
                                } else if (activePaymentMethod === 'PAYSTACK') {
                                    setActivePaymentMethod(isOfflineEnabled ? 'OFFLINE' : 'STRIPE');
                                } else {
                                    setActivePaymentMethod(isStripeEnabled ? 'STRIPE' : 'PAYSTACK');
                                }
                            }}
                            style={{ cursor: 'pointer' }}
                        >
                            {activePaymentMethod === 'STRIPE'
                                ? t`I would like to pay using a different method`
